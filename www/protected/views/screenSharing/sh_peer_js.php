...
Yii::app()->clientScript->registerScript(uniqid('sh_peer_js'), "
	...
	ScreenSharingPeer.prototype._validateRequirementsAndGetUniversalObjects = function()
	{
		...
		if (!location.protocol.match('https')) return '".Yii::t('general', 'You need to run this application from https')."';
		
		if (navigator.userAgent.match('Chrome'))
		{
			var browserMajorVersion = parseInt(navigator.userAgent.match(/ Chrome\/(.*)/)[1]);
			
			if (browserMajorVersion < 26) return '".Yii::t('general', 'You need Chrome 26+ to run this application')."';
		...
		else if (navigator.userAgent.match('Firefox'))
		{
			...
			if (browserMajorVersion < 24) return '".Yii::t('general', 'You need Firefox 24+ to run this application')."';
		...
		{
			return '".Yii::t('general', 'This application needs to be executed in Firefox or Chrome')."';
		}
